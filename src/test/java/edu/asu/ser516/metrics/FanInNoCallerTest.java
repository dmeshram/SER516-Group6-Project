...
public class FanInNoCallerTest {

        private static final Path FIXTURE = Path.of("src/test/resources/fixtures/fanin_no_callers");

        @Test
        @DisplayName("Method with no callers appears in result with fanIn = 0")
        void methodWithNoCallersHasFanInZero() {
                Map<String, Integer> fanInByMethod = new FunctionFanInComputer().compute(FIXTURE);
        ...
        void allMethodsInIsolatedClassHaveFanInZero() {
                ...
                fanInByMethod.forEach((method, fanIn) -> assertEquals(0, fanIn,
